  ...
  try {
    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          // Search for manufacturer/company in the entire HTML
          const lowerHtml = record.rawHtml.toLowerCase();
          const manufacturerIdx = lowerHtml.indexOf('manufacturer');
          const companyIdx = lowerHtml.indexOf('company');
          const applicantIdx = lowerHtml.indexOf('applicant');
          
          console.log('manufacturer index:', manufacturerIdx);
          console.log('company index:', companyIdx);
          console.log('applicant index:', applicantIdx);
          
          if (manufacturerIdx > 0) {
            console.log('\nManufacturer context:', record.rawHtml.substring(manufacturerIdx - 50, manufacturerIdx + 200));
          }
          if (companyIdx > 0) {
            console.log('\nCompany context:', record.rawHtml.substring(companyIdx - 50, companyIdx + 200));
          }
          if (applicantIdx > 0) {
            console.log('\nApplicant context:', record.rawHtml.substring(applicantIdx - 50, applicantIdx + 200));
          }
        }
      }
    }
