      ...
      {filtered.map((item) => {
        ...
        return (
          <article key={id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <textarea value={notes[id] || ""} onChange={(event) => setNotes({ ...notes, [id]: event.target.value })} placeholder="Review notes and source evidence summary" className="mt-3 w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm" />
