    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          ...
          const patterns = [
            /<span[^>]*>Company Name<\/span>/i,
            /<span[^>]*>Company Address<\/span>/i,
            /<span[^>]*>Manufacturer<\/span>/i,
            /<span[^>]*>Applicant<\/span>/i,
          ];
          
          for (const pattern of patterns) {
            const idx = record.rawHtml.search(pattern);
            if (idx >= 0) {
              console.log(`\nFound pattern "${pattern.source}" at index ${idx}`);
              console.log('Context:', record.rawHtml.substring(idx, idx + 500));
            }
          }
        }
      }
