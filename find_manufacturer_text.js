  ...
  try {
    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          // Look for all text in the HTML to find manufacturer info
          const textContent = record.rawHtml.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '')
            .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '')
            .replace(/<[^>]+>/g, ' ')
            .replace(/\s+/g, ' ')
            .trim();
          
          // Find sections containing manufacturer, company, applicant
          const lowerText = textContent.toLowerCase();
          
          // Look for section headers
          const sectionPattern = /(manufacturer|company|applicant|importer|packer|labeler)[^:]*:?\s*([^,\n]+)/gi;
          let match;
          while ((match = sectionPattern.exec(textContent)) !== null) {
            console.log(`  Found: ${match[1].trim()}: ${match[2].trim().substring(0, 100)}`);
          }
        }
      }
    }
