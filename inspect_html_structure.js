    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          ...
          const tableRows = record.rawHtml.match(/<tr[^>]*>.*?<\/tr>/gi);
          if (tableRows) {
            for (const row of tableRows.slice(0, 30)) {
              const cells = row.match(/<t[d|th][^>]*>(.*?)<\/t[d|th]>/gi);
              if (cells) {
                console.log('  Row:', cells.map(c => c.replace(/<[^>]+>/g, '').trim()).join(' | '));
              }
            }
          }
        }
