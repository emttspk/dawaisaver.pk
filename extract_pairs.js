  ...
  try {
    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          // Look for label/value pairs with the specific structure
          const labelValuePairs = record.rawHtml.match(/<span[^>]*class="[^"]*d-block[^"]*"[^>]*>\s*([^<]+)\s*<\/span>\s*<div[^>]*>\s*([^<]+)\s*<\/div>/gi);
          
          if (labelValuePairs) {
            for (const pair of labelValuePairs) {
              const labelMatch = pair.match(/<span[^>]*>\s*([^<]+)\s*<\/span>/i);
              const valueMatch = pair.match(/<div[^>]*>\s*([^<]+)\s*<\/div>/i);
