...
(async () => {
  try {
    ...
    let manufacturerNameCount = 0;
    let applicantNameCount = 0;
      ...
      if (line.trim()) {
        try {
          ...
          if (record.rawHtml) {
            ...
            const isContract = lowerHtml.includes('contract') && !lowerHtml.includes('self manufacturing');
            
            if (isContract) {
              contractCount++;
              const hasCompanyName = record.rawHtml.includes('Company Name');
              const hasManufacturerName = record.rawHtml.includes('Manufacturer Name');
              const hasApplicantName = record.rawHtml.includes('Applicant Name');
              
              if (hasCompanyName) companyNameCount++;
              if (hasManufacturerName) manufacturerNameCount++;
              if (hasApplicantName) applicantNameCount++;
            }
          }
        } catch (e) {}
      }
    ...
    console.log('Contract/Import records:', contractCount);
    console.log('Records with Company Name:', companyNameCount);
    console.log('Records with Manufacturer Name:', manufacturerNameCount);
    console.log('Records with Applicant Name:', applicantNameCount);
  } catch (e) {
