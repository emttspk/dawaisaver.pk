  ...
  try {
    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          // Look for "Manufacturer (Contract / Import only)" section
          const sectionIdx = record.rawHtml.indexOf('Manufacturer (Contract');
          if (sectionIdx > 0) {
            console.log('\nManufacturer section found');
            console.log(record.rawHtml.substring(sectionIdx, sectionIdx + 1000));
          }
          
          // Also look for MAH section
          const mahIdx = record.rawHtml.indexOf('MAH (Import only)');
          if (mahIdx > 0) {
            console.log('\nMAH section found');
            console.log(record.rawHtml.substring(mahIdx, mahIdx + 1000));
          }
        }
      }
    }
