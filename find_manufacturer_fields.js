  ...
  try {
    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          // Search for company/manufacturer/applicant fields
          const patterns = [
            /Company\s*[:\-]\s*([^<\n]+)/i,
            /Manufacturer\s*[:\-]\s*([^<\n]+)/i,
            /Applicant\s*[:\-]\s*([^<\n]+)/i,
            ...
            /Manufacturer\/Importer\s*[:\-]\s*([^<\n]+)/i,
            /Address\s*[:\-]\s*([^<\n]+)/i,
            /Company\s*Address\s*[:\-]\s*([^<\n]+)/i,
          ];
          
          for (const pattern of patterns) {
            const match = record.rawHtml.match(pattern);
            if (match) {
              console.log(`  Found (${pattern.source}):`, match[1].trim().substring(0, 100));
            }
          }
        }
      }
