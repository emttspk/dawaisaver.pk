...
(async () => {
  try {
    ...
    // Get first contract/import record and show all labels
        ...
        try {
          ...
          if (record.rawHtml) {
            ...
            if (isContract) {
              ...
              const labelPattern = /<div[^>]*class="[^"]*col-[^"]*"[^>]*>[\s\S]*?<div[^>]*class="[^"]*fw-semibold[^"]*"[^>]*>\s*([^<]+)\s*<\/div>\s*<div[^>]*>\s*([^<]+)\s*<\/div>/gi;
              let match;
              while ((match = labelPattern.exec(record.rawHtml)) !== null) {
