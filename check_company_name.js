    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          ...
          const companyNameIdx = record.rawHtml.indexOf('Company Name');
          const companyAddressIdx = record.rawHtml.indexOf('Company Address');
          const manufacturerIdx = record.rawHtml.indexOf('Manufacturer');
          const applicantIdx = record.rawHtml.indexOf('Applicant');
          
          console.log('Company Name index:', companyNameIdx);
          console.log('Company Address index:', companyAddressIdx);
          console.log('Manufacturer index:', manufacturerIdx);
          console.log('Applicant index:', applicantIdx);
          
          if (companyNameIdx > 0) {
            console.log('\nCompany Name context:', record.rawHtml.substring(companyNameIdx, companyNameIdx + 300));
          }
          if (companyAddressIdx > 0) {
            console.log('\nCompany Address context:', record.rawHtml.substring(companyAddressIdx, companyAddressIdx + 300));
          }
        }
      }
    }
