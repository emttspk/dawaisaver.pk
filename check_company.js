  ...
  try {
    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (record.rawHtml) {
          // Check if "Company" appears anywhere
          if (record.rawHtml.includes('Company')) {
            console.log('FOUND Company in HTML');
            // Find context around "Company"
            const idx = record.rawHtml.indexOf('Company');
            console.log('Context around Company:', record.rawHtml.substring(idx - 100, idx + 300));
          }
          if (record.rawHtml.includes('company')) {
            console.log('FOUND company (lowercase) in HTML');
            const idx = record.rawHtml.indexOf('company');
            console.log('Context:', record.rawHtml.substring(idx - 100, idx + 300));
          }
        }
      }
    }
